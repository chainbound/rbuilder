...
pub enum RawBundleConvertError {
    ...
    #[error("Incorrect replacement data: {0}")]
    IncorrectReplacementData(&'static str),
                ...
                let (sequence_number, signer) = match (replacement_nonce, first_seen_at_secs) {
                    (None, None) => {
                        return Err(RawBundleConvertError::IncorrectReplacementData(
                            "no replacement_nonce and first_seen_at_secs provided",
                        ))
                    }
                    ...
                    (Some(replacement_nonce), None) => match signing_address {
                        ...
                        None => {
                            return Err(RawBundleConvertError::IncorrectReplacementData(
                                "replacment_nonce provided but no signing_address",
                            ))
                        }
                    },
                    (Some(_), Some(_)) => {
                        return Err(RawBundleConvertError::IncorrectReplacementData(
                            "both replacement_nonce and first_seen_at_secs provided",
                        ))
                    }
                };
